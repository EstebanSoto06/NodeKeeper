...
npm run test:frontend   # Vitest + React Testing Library, 141 pruebas
```
...
MVP local funcional: autenticación, roles, catálogos, mantenimientos con checklist y evidencias, vistas operativas (dashboard, calendario, mapa real con Leaflet/OpenStreetMap, reportes) y gestión de usuarios están implementados y probados (245 pruebas backend, 141 pruebas frontend). El historial de mantenimiento está protegido ante eliminación de nodos/equipos, y la API aplica rate limiting general y estricto en el login. El despliegue público queda fuera de este alcance.
